...
def preprocess_sequences(X, max_length=150):
    processed = []
    for seq in X:
        # If sequence is too long, truncate from the end
        if len(seq) > max_length:
            seq = seq[:max_length]
        # If sequence is too short, pad from the left
        else:
            seq = seq.zfill(max_length)
        encoded = padded_one_hot_encode(seq)
...
def plot_scatter(observed, predicted):
    ...
    plt.title('Observed vs. Predicted Transcription Rate')
    plt.xlabel('Observed Transcription Rate')
    plt.ylabel('Predicted Transcription Rate')
    plt.legend()
    plt.grid(True)
    plt.show()
