...
def load_df(file_path):
    ...
    df['Reversed Normalized Observed TX/Txref'] = 1 - MinMaxScaler().fit_transform(np.exp(df[['Observed log(TX/Txref)']]))
    return df
...
def load_features(file_path):
    ...
    y = df['Reversed Normalized Observed TX/Txref']
    return X, y
...
def plot_kde(observed, predicted):
    sns.kdeplot(observed, fill=True, color='blue', label='Reversed Normalized Observed TX/Txref')
