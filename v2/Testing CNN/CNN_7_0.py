...
def load_and_preprocess_data(file_path):
    ...
    df['Normalized Augmented Observed log(TX/Txref)'] = (MinMaxScaler().fit_transform(df[['Augmented Observed log(TX/Txref)']] * -1) * 2) - 1
    return df

def combine_columns(df):
    X = df[['Promoter Sequence']].astype(str).agg(''.join, axis=1)
    y = df['Normalized Augmented Observed log(TX/Txref)']
    return X, y
...
def plot_kde(df, predicted):
    sns.kdeplot(df['Normalized Augmented Observed log(TX/Txref)'], fill=True, color='blue', label='Observed log(TX/Txref)')
